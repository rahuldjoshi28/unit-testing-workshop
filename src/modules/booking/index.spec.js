...
test("BookMovieTicket should get render and should also use MovieBookingForm as child with handleSubmit", async () => {
    const bookTicket = jest.spyOn(movieServices, "bookTicket").mockResolvedValue({ seatNumber: "1", ticketNumber: "2" })
    ...
    await waitFor(() => expect(window.location.search).toBe("?seat-number=1&ticket-number=2"))
})